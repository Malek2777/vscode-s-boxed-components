...
function activate(context) {
	...
	const quickPickWorkspace =  async () => {
		...
		const templates = configuration.get('s-boxed-components.useTemplates');
	...
	const createBox = vscode.commands.registerCommand('s-boxed-components.createBox', () => {
		...
		if (vscode.workspace.workspaceFolders.length > 1) {
			...
			quickPickTemplate(vscode.workspace.getConfiguration('s-boxed-components').get('useTemplates'), vscode.workspace.workspaceFolders[0].uri.fsPath);
		}
